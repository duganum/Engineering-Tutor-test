...
def render_problem_diagram(prob_id):
    ...
    # ---------------------------------------------------------
    elif pid == "K_2.1_1": # Velocity Graph
        t = np.linspace(0, 6, 100); v = 20*t**2 - 100*t + 50
        ax.plot(t, v, 'b-'); ax.plot(2.5, -75, 'ro')
        ax.set_xlim(0, 6); ax.set_ylim(-100, 200); ax.set_aspect('auto')
        found = True
    elif pid == "K_2.1_2": # Cliff Image (Preserve Aspect)
        try:
    ...
    elif pid == "K_2.1_3": # Runner graph
        ax.plot([0, 0.889, 4.0], [0, 22.5, 22.5], 'g-', lw=2)
        ax.set_xlim(-0.5, 4.5); ax.set_ylim(-5, 30); ax.set_aspect('auto')
        found = True
    elif "K_2.2" in pid: # Projectiles
        x = np.linspace(0, 4, 100); y = -0.5*(x-2)**2 + 2
        ax.plot(x, y, 'k--'); found = True
        if "3" in pid: ax.plot([-0.5, 0], [0, 2], 'k-', lw=3)
    elif "K_2.3" in pid: # Normal/Tangent
        c = plt.Circle((0,0), 1.5, fill=False, ls='--'); ax.add_patch(c)
        ax.annotate('', xy=(0,0), xytext=(1, 1), arrowprops=dict(arrowstyle='->', color='red'))
        found = True
    elif "K_2.4" in pid: # Polar
        ax.plot([0, 1.5], [0, 1.2], 'k-o', lw=4)
        ax.annotate('', xy=(2, 1.6), xytext=(1.5, 1.2), arrowprops=dict(arrowstyle='->', color='blue'))
        found = True
